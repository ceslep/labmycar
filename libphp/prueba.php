...
$urlLogo = $dato_conf['urlLogoLaboratorio'] ?? '';
        ...
        <div class="text-center mb-8">
            <div class="bg-indigo-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                <?php if (!empty($urlLogo)): ?>
                    <img src="<?= str_starts_with(trim($urlLogo), 'data:image') ? $urlLogo : htmlspecialchars($urlLogo) ?>" alt="Logo" class="w-10 h-10">
                <?php endif; ?>
            </div>
...
<body class="bg-slate-100 h-screen flex flex-col overflow-hidden" x-data="labApp('<?= $identificacion_inicial ?>')">
    <header class="bg-gradient-to-r from-indigo-800 to-purple-800 text-white p-3 shadow-lg">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <div class="flex items-center gap-3">
                <?php if (!empty($urlLogo)): ?>
                    <img src="<?= str_starts_with(trim($urlLogo), 'data:image') ? $urlLogo : htmlspecialchars($urlLogo) ?>" alt="Logo" class="h-8 w-8 object-contain">
                <?php else: ?>
            ...
            <div class="flex items-center gap-4">
                <button @click="testFunciones()"
                    class="bg-red-600 hover:bg-red-700 px-2 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 shadow-md"
                    title="Probar funciones">
                    <i class="bi bi-bug"></i>
                            ...
                            <div x-show="idAbierto === '<?= $id_p ?>_<?= $fecha_examen ?>'" x-cloak
                                ...
                                <div class="p-4 space-y-4 bg-gradient-to-b from-slate-50 to-white rounded-b-2xl">
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <?php
                                        $url_todo = "printphp/imprimirTodo.php?idx=" . bin2hex(random_bytes(10)) . "&identificacion=$id_p&fecha=$fecha_examen&nombres=" . urlencode($nom_p) . "&edad=$edad_p&info=Resultados&ver=1";
                                        ?>
        ...
        <main class="flex-1 relative flex flex-col bg-slate-200">
            ...
            <div x-show="!urlReporte" x-cloak class="flex-1 flex flex-col items-center justify-center p-8">
                <div class="text-center">
                    <?php if (!empty($urlLogo)): ?>
                        <img src="<?= str_starts_with(trim($urlLogo), 'data:image') ? $urlLogo : htmlspecialchars($urlLogo) ?>" alt="Logo"
    ...
    <div x-show="mostrarModalPacientesBusqueda" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-6xl w-full h-[85vh] flex flex-col border-4 border-green-500" @click.stop>
                ...
                <form @submit.prevent="buscarPacientes()" class="p-6 space-y-5 overflow-y-auto flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" x-model="formularioPacientesBusqueda.telefono"
                        ...
                        <div>
                            ...
                            <input type="text" x-model="formularioPacientesBusqueda.ciudad"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" x-model="formularioPacientesBusqueda.entidad"
                                placeholder="Nombre de la entidad"
                                class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                <i class="bi bi-sliders text-green-600"></i>
                                Límite de resultados
                            </label>
                            <select x-model="formularioPacientesBusqueda.limit"
                                class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all">
                                <option value="25">25 pacientes</option>
                                <option value="50">50 pacientes</option>
                                <option value="100">100 pacientes</option>
                                <option value="200">200 pacientes</option>
                            </select>
                        </div>
                    </div>
                    <!-- Opciones adicionales -->
                    <div class="bg-green-50 p-4 rounded-xl border border-green-100">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <span class="text-sm font-semibold text-green-800">Opciones de búsqueda</span>
                        </div>
                        <div class="space-y-3">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" x-model="formularioPacientesBusqueda.includeExamenes"
                                    class="h-4 w-4 text-green-600 focus:ring-green-500 border-slate-300 rounded">
                                <span class="text-sm text-slate-700">Incluir exámenes del paciente</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" x-model="formularioPacientesBusqueda.soloConResultados"
                                    class="h-4 w-4 text-green-600 focus:ring-green-500 border-slate-300 rounded">
                                <span class="text-sm text-slate-700">Solo pacientes con exámenes completados</span>
                            </label>
                        </div>
                    </div>
                    <!-- Botones de acción -->
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-emerald-100 text-sm"
                                    x-text="`${resultadosPacientesBusqueda?.total_pacientes || 0} pacientes encontrados`"></p>
                            </div>
                        </div>
                        <div class="flex gap-2">
                            ...
                            <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden paciente-card"
                                ...
                                <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-4 border-b border-green-100">
                                    <div class="flex justify-between items-start">
                                        <div class="flex gap-3">
                                            ...
                                            <div>
                                                <h4 class="font-bold text-slate-800 text-lg" x-text="paciente.nombre_completo"></h4>
                                                <div class="flex flex-wrap gap-2 mt-1">
                                                    <span class="chip chip-identificacion" x-text="paciente.identificacion"></span>
                                                    <span class="chip chip-genero-masculino" x-text="`${paciente.edad} años`"></span>
                                                    <span class="chip chip-telefono" x-text="paciente.telefono || 'Sin teléfono'"></span>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            <button @click="seleccionarPaciente(paciente)"
                                ...
                                <div class="p-4">
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                                        <div class="text-center">
                                            <div class="text-lg font-bold text-slate-800" x-text="paciente.total_visitas || 0"></div>
                                            <div class="text-xs text-slate-500 uppercase">Visitas</div>
                                        </div>
                                        <div class="text-center">
                                            <div class="text-lg font-bold text-slate-800" x-text="paciente.total_examenes || 0"></div>
                                            <div class="text-xs text-slate-500 uppercase">Exámenes</div>
                                        </div>
                                        <div class="text-center">
                                            <div class="text-lg font-bold text-slate-800" x-text="paciente.examenes_con_resultados || 0"></div>
                                            <div class="text-xs text-slate-500 uppercase">Con resultados</div>
                                        </div>
                                        <div class="text-center">
                                            <div class="text-sm font-bold text-slate-800" x-text="paciente.ultima_visita || 'N/A'"></div>
                                    ...
                                    <div x-show="paciente.examenes && paciente.examenes.length > 0">
                                        ...
                                        <div x-show="examenesExpandidos[index]" x-transition class="space-y-2">
                                            <template x-for="(examen, examenIndex) in paciente.examenes" :key="examenIndex">
                                                <div class="bg-slate-50 rounded-lg p-3 flex items-center justify-between">
                                                    <div class="flex-1">
                                                        <div class="font-medium text-slate-800 text-sm" x-text="examen.nombre"></div>
                                                        <div class="text-xs text-slate-500">
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('labApp', (idInicial) => ({
                ...
                async actualizarEntidad(selectElement) {
                    ...
                    if (!identificacion || !fecha_examen) {
                        Swal.fire('Error', 'Datos de paciente o fecha incompletos.', 'error');
                        return;
                    }
                    const result = await Swal.fire({
                        title: '¿Confirmar cambio de entidad?',
                        text: `Se actualizará la entidad para ${identificacion} en la fecha ${fecha_examen}.`,
                        icon: 'question',
                ...
                seleccionarPaciente(paciente) {
                    this.pacienteSeleccionado = this.pacienteSeleccionado?.identificacion === paciente.identificacion ? null : paciente;
                },
                toggleExamenExpandido(examenIndex) {
                    this.$set(this.examenesExpandidos, examenIndex, !this.examenesExpandidos[examenIndex]);
                },
